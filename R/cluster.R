...
makeCluster <- function(clusterSetting = "cluster_settings.json", fullName = FALSE, wait = TRUE, resourceFiles = list()){
  ...
  if(!is.null(pool$rPackages) && !is.null(pool$rPackages$github) && length(pool$rPackages$github) > 0){
    if(is.null(packages)){
      packages <- getGithubInstallationCommand(pool$rPackages$github, pool$rPackages$github_auth)
    }
    else{
      packages <- paste0(packages, ";", getGithubInstallationCommand(pool$rPackages$github, pool$rPackages$github_auth))
    }
  }
