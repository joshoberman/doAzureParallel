...
getURLInstallationCommand <- function(packages){
  ...
  if(length(packages) != 0){
    for(package in packages){
      installation <- paste0(installation,
                             sprintf("Rscript -e \'args <- commandArgs(TRUE)\' -e \'devtools::install_url(args[1])\' %s", package),
                             ";")
    }
  }
...
getVersionInstallationCommand <- function(packages, versions){
  installation <- ""
  installation <- paste0(installation,
                         sprintf("Rscript -e \'args <- commandArgs(TRUE)\' -e \'install.packages(args[1], dependencies=TRUE)\' %s", "devtools"),
                         ";")
  ...
  if(length(packages) != 0){
    for(package in packages){
      installation <- paste0(installation,
                             sprintf("Rscript -e \'args <- commandArgs(TRUE)\' -e \'devtools::install_version(args[1])\' %s %s", package),
                             ";")
    }
  }
